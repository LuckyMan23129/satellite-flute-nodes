...
#include "switch_sensor_power.h"
#include "config.h"

static int8_t ret;

LOG_MODULE_REGISTER(switch_sensor_power);

#define PERI_SW DT_ALIAS(sensorsw)

// Get the device pointer, pin number, and pin's configuration flags through gpio_dt_spec
static const struct gpio_dt_spec sensor_sw  = GPIO_DT_SPEC_GET(PERI_SW, gpios);
...
int8_t check_gpio_sensor_sw(void){
    if (!device_is_ready(sensor_sw.port)) {
        LOG_ERR("GPIO initialization for Digital SW is not successful!");
        return (1);
    }

    // ret = gpio_pin_configure_dt(&sensor_sw, GPIO_PULL_UP | GPIO_OUTPUT_ACTIVE);
    ret = gpio_pin_configure_dt(&sensor_sw, GPIO_OUTPUT_INACTIVE);
    if (ret < 0) {
