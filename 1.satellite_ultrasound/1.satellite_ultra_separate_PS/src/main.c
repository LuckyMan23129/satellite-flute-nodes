...
static const astar_config_t astar_params = {
    ...
    .daily_initial_sleep_SleepTime  = 120,
...
{   
    ...
    if (check_gpio_sensor_sw()) {
        if (ENABLE_PRINT)
            LOG_INF("Sensor power switch GPIO not found in Devicetree");
        k_sleep(K_FOREVER);
    }
    ...
    rerun_astar_after_suspension:
        ...
        // 2. Sensor reading
        //====================================================================
        periph_sw_turn_on();        // Power on the Satellite Modem
        sensor_sw_turn_on();        // power on the Sensor
        k_sleep(K_MSEC(2000));              // Vu - MUST EQUAL OR HIGHER THIS DELAY - Wait for sensor power stabilisation
        distance = get_safe_distance_cm();

        sensor_sw_turn_off();       // Power off the sensor to save energy
