                    ...
                    void                *user_data)
{
    ...
    case UART_RX_RDY: {
        ...
        if (ENABLE_PRINT) LOG_DBG("RX [%u bytes]: %.*s", (unsigned int)len, (int)len, (const char *)src);
        ...
        if (expected_response != NULL) {
            ...
            if (strstr(resp_buf, expected_response) != NULL) {
                if (ENABLE_PRINT) LOG_DBG("Response matched: '%s'", expected_response);
...
int iridium_uart_init(void)
{
    ...
    if (ENABLE_PRINT) LOG_DBG("UART2 device: %p", uart_dev);
    ...
    if (ENABLE_PRINT) LOG_INF("Iridium UART initialised (semaphore mode) — waiting for modem ready...");
    ...
    if (ENABLE_PRINT) LOG_INF("Modem ready");
                                   ...
                                   int         timeout_s)
{
    ...
    if (ENABLE_PRINT) LOG_DBG("Waiting for response (blocking on semaphore, timeout %d s)...", timeout_s);
    ...
    if (ret == 0) {
        ...
        if (ENABLE_PRINT) LOG_INF("Response received: %s", iridium_result_str(response_result));
                                           ...
                                           int            timeout_s)
{
    ...
    if (ENABLE_PRINT) LOG_INF("CMD: AT+SBDWB=%u (%u data bytes + 2-byte checksum)", (unsigned int)len, (unsigned int)len);
    ...
    if (ENABLE_PRINT) LOG_DBG("Sending %u bytes of data + 2-byte checksum (0x%04X)", (unsigned int)len, cs);
    ...
    if (ENABLE_PRINT) LOG_DBG("Waiting for binary write result (semaphore)...");
    ret = k_sem_take(&response_sem, K_SECONDS(timeout_s));
    expected_response = NULL;
    ...
    if (strstr(resp_buf, "0") != NULL) {
        if (ENABLE_PRINT) LOG_INF("Binary payload accepted by modem (%u bytes)", (unsigned int)len);
        return IRIDIUM_OK;
    }
...
iridium_result_t iridium_sbd_initiate_session(int timeout_s)
{
    ...
    if (ENABLE_PRINT) LOG_INF("CMD: AT+SBDIX (timeout %d s)", timeout_s);
    ...
    if (result != IRIDIUM_OK) {
        if (ENABLE_PRINT) LOG_INF("AT+SBDIX completed: %s", iridium_result_str(result));
        return result;
    }
                                             ...
                                             size_t         len)
{
    ...
    if (result == IRIDIUM_OK) {
        if (ENABLE_PRINT) LOG_INF("Satellite transmission successful!");
    } else {
        LOG_ERR("Satellite transmission failed (%s)",
                iridium_result_str(result));
        if (ENABLE_PRINT) LOG_INF("Hint: a clear sky view is required for satellite contact");
    }
...
{
    if (ENABLE_PRINT) LOG_INF("Testing basic modem communication (AT)...");
    ...
    if (ENABLE_PRINT) LOG_INF("Disabling flow control (AT&K0)...");
    ...
    if (ENABLE_PRINT) LOG_INF("Modem is responding correctly");
    return IRIDIUM_OK;
}
...
void iridium_send_binary_frame(const uint8_t *frame, size_t len)
{
    ...
    if (result == IRIDIUM_OK) {
        if (ENABLE_PRINT) LOG_INF("Completed  transmission successfully");
    } else {
...
int iridium_get_signal_quality(void)
{
    ...
    if (ENABLE_PRINT) LOG_INF("Reading signal quality (AT+CSQ)...");
    ...
    if (sscanf(csq_line, "+CSQ:%d,%d", &rssi, &ber) >= 1) {
        if (ENABLE_PRINT) LOG_INF("Signal quality: RSSI=%d, BER=%d", rssi, ber);

        /* Validate and log signal strength */
        if (rssi >= 0 && rssi <= 31) {
            if (ENABLE_PRINT) LOG_INF("  Signal strength: %s",
...
int iridium_get_imei(char *buf, size_t buf_len)
{
    ...
    if (ENABLE_PRINT) LOG_INF("Reading IMEI (AT+CGSN)...");
    ...
    if (ENABLE_PRINT) LOG_INF("IMEI: %s", buf);
...
iridium_result_t iridium_send_text(const char *msg, int timeout_s)
{
    ...
    if (ENABLE_PRINT) LOG_INF("Writing text to SBD TX buffer: \"%s\"", msg);
    ...
    if (ENABLE_PRINT) LOG_INF("Initiating satellite exchange (AT+SBDIX)...");
    LOG_WRN("Satellite acquisition may take up to %d s", TIMEOUT_SBDIX_S);

    result = iridium_sbd_initiate_session(TIMEOUT_SBDIX_S);

    if (result == IRIDIUM_OK) {
        if (ENABLE_PRINT) LOG_INF("Text message sent successfully");
    } else {
